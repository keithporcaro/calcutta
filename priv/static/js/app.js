...
var App = function () {
  ...
  _createClass(App, null, [{
    ...
    value: function init() {
      ...
      $bid.off("keypress").on("keypress", function (e) {
        if (e.keyCode == 13) {
      ...
      chan.on("new:bid", function (bid) {
        $messages.append(_this.bidTemplate(bid));
        scrollTo(0, document.body.scrollHeight);
      });

      chan.on("user:entered", function (msg) {
        var username = _this.sanitize(msg.user || "bidder");
    ...
    key: "bidTemplate",
    value: function bidTemplate(bid) {
      var username = this.sanitize(bid.user || "anonymous");
      var body = this.sanitize(bid.body);
